...
if __name__ == "__main__":
    ...
    def enum_name(func: Func):
        return 'pmap_enum_' + func.name.split('pmap_')[1] # todo maybe switch
    ...
    def dispatch_line(func: Func):
        case_line = '\n\t\tcase ' + enum_name(func) + ':'
        call = zoned_name(func) + '('
        for idx, argtype in enumerate(func.argtypes):
            call += '(' + argtype + ') ' + 'call->args[' + str(idx) + '], '
        call = call[:-2] + ');'

        case_body = '\n\t\t\treturn (' + transfer_type + ') ' + call
        if func.rettype == 'void':
            case_body = '\n\t\t\t' + call + '\n\t\t\treturn NULL;'#todo switch to 0
    ...
    def public_fn(func: Func):
        fn_str = '\n' + func.rettype + '\n' + func.name + '('
        pmap_call = '\n\tstruct pmap_call call = {' + enum_name(func) + ', '
        for idx in range(len(func.argtypes)):
            argtype = func.argtypes[idx]
            argname = func.argnames[idx]
            fn_str += argtype + ' ' + argname + (' __unused' if func.unused_macro[idx] else '') + ', '
            pmap_call += '(' + transfer_type + ') ' + argname + ', '
        pmap_call = pmap_call[:-2] + '};'
        fn_str = fn_str[:-2] + ')\n{' + pmap_call
        
        zm_call = 'zm_zone_enter(ZONE_STATE_PMAP, (void *)&call);'# todo add space
        if func.rettype != 'void':
    ...
    with open('pmap_candidate.c', 'w') as pmap_candidate:
        pmap_candidate.write(pmapc)
        pmap_candidate.write(additions())
